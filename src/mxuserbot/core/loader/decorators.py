...
def state(target_state):
    def decorator(func):
        func.is_state = True
        func.target_state = (
            target_state.state if hasattr(target_state, "state") else target_state
        )
        return func
...
def _collect_commands(cls, paths):
    ...
    mod_path = sys.modules[cls.__module__].__file__
    mod_file = pathlib.Path(mod_path).parent
...
def _tds_impl(cls):
    ...
    if is_legacy:
        ...
        for command_, func in cutils.get_commands(cls).items():
            ...
            if cmd_doc:
                cls.strings.setdefault(
                    f"_cmd_doc_{command_}", inspect.cleandoc(cmd_doc)
                )
