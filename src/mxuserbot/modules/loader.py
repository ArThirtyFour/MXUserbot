...
class LoaderModule(loader.Module):
    config = {
        "repo_url": loader.ConfigValue(
            "https://raw.githubusercontent.com/MxUserBot/mx-modules/main",
            "Main system repository URL",
            required=True,
        ),
        "first_unsafe_warn_ok": loader.ConfigValue(
            False, "User read and accepted the community module security warning"
        ),
        "dev_warn_ok": loader.ConfigValue(
            False, "User accepted dev/file installation warning"
        ),
    }
    ...
    async def _security_gate(
        ...
        await self._confirm_unsafe_install(
            mx, event, source_url or payload.target, on_confirm=on_confirm
        )
        return False
    ...
    async def msearch(self, mx, event: MessageEvent, payload: SearchPayload):
        ...
        if not results:
            return await utils.answer(
                mx, self.strings["search_empty"].format(query=payload.query)
            )
        ...
        for res in results:
            ...
            for mod in res["modules"]:
                flat_list.append({"repo_info": res, "mod_info": mod, "prefix": prefix})
                ...
                if res["repo_url"] != last_repo_url:
                    if res["is_verified"]:
                        header = self.strings["search_header"].format(
                            icon="✅", type="SYSTEM", url=res["repo_url"]
                        )
                    else:
        ...
        for i in range(0, total_mod_count, per_page):
            pages.append(
                render_page(flat_list[i : i + per_page], len(pages), total_pages_count)
            )
    ...
    async def mdl(self, mx, event: MessageEvent, payload: MdlPayload):
        ...
        if reply_event:
            ...
            async def _install(ctx):
                ...
                if await self.repo.install(**install_kw, event=event):
                    await utils.answer(
                        mx, self.strings["done"].format(name=fname), edit_id=status_id
                    )
                    await self.loader.show_module_help(mx, event, fname)
                else:
                    await utils.answer(
                        mx,
                        self.strings["error"].format(err="Install failed!"),
                        edit_id=status_id,
                    )

            if not await self._security_gate(
                mx,
                event,
                payload,
                False,
        ...
        status_id = await utils.answer(
            mx, self.strings["fetching"].format(id=payload.target[:20])
        )
        ...
        if not url:
            return await utils.answer(
                mx,
                self.strings["repo_not_found"].format(id=payload.target),
                edit_id=status_id,
            )
        ...
        async def _install(ctx):
            ...
            if await self.repo.install(target=payload.target, event=event):
                ...
                if not filename.endswith((".py", ".zip")):
                    filename += ".py"
                await utils.answer(
                    mx, self.strings["done"].format(name=filename), edit_id=status_id
                )
        ...
        if not await self._security_gate(
            ...
            event,
            payload,
            getattr(source, "is_verified", False),
            source_url=url,
            on_confirm=_install,
        ):
            return
    ...
    async def addrepo(self, mx, event: MessageEvent, payload: RepoPayload):
        ...
        try:
            ...
            await utils.answer(
                mx, self.strings["confirm_cancelled"], edit_id=event.event_id
            )
    ...
    async def reload(self, mx, event: MessageEvent):
        ...
        if errors:
            error_lines = [
                f"  ⚠️ <b>{e['name']}</b>: <code>{cutils.escape_html(e['error'])}</code>"
                for e in errors
            ]
    ...
    async def update(self, mx, event: MessageEvent, payload: UpdatePayload):
        ...
        if payload.name.lower() == "all":
            ...
            if not updates:
                await utils.answer(
                    mx, "✅ | <b>All modules are up to date.</b>", edit_id=status_id
                )
                return
            success = []
            failed = []
            for upd in updates:
                ...
                old_code = (
                    old_file.read_text(encoding="utf-8") if old_file.exists() else None
                )
                try:
                    ...
                    if old_code is not None:
                        try:
                            ...
                            await self.loader.register_module(
                                old_file, mx, is_core=False
                            )
            ...
            if failed:
                msg += "<br><br><b>Failed:</b><br>" + "<br>".join(
                    f"  ⚠️ <code>{cutils.escape_html(str(f))}</code>" for f in failed
                )
            await utils.answer(mx, msg, edit_id=status_id)
            return
        ...
        if not url:
            await utils.answer(
                mx, self.strings["repo_not_found"].format(id=payload.name)
            )
            return
        status_id = await utils.answer(
            mx, self.strings["fetching"].format(id=payload.name)
        )
        try:
            if await self.repo.install(target=payload.name, event=event):
                ...
                await utils.answer(
                    mx,
                    self.strings["done"].format(name=payload.name),
                    edit_id=status_id,
                )
                await self.loader.show_module_help(mx, event, filename)
            else:
                await utils.answer(
                    mx,
                    self.strings["error"].format(err="Update failed!"),
                    edit_id=status_id,
                )
        except Exception as e:
            await utils.answer(
                mx, self.strings["error"].format(err=str(e)), edit_id=status_id
            )
